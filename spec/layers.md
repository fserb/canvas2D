...
To apply a filter, shadow, blur or compositing to a set of draw operations (as opposed to a single one) the only current solution is draw to a temporary canvas and then draw the temporary canvas into the final destination.
